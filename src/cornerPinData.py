...
class CornerPinDataFromNukeCornerPinText(CornerPinDataAsList):
    ...
    def __parseMochaNukeData(self, trackingData):
        ...
        def parseCornerPoint(pointName,trackingData):
            parseDataRegExp =  re.compile(pointName+r"\s*{\s*{\s*curve\s*([^}]*)}\s*{\s*curve\s*([^}]*)}\s*}")
            trackingDataParse = parseDataRegExp.search(trackingData);
            if trackingDataParse == None:
                raise FileFormatError("invalid tracking data - please use corner pin data exported from mocha Pro with the format set to 'Nuke Corner Pin (*.nk)'")
