...
def createTransformNodeMI():
    ...
    loadTrackingDataTab.addLoadTrackingDataUI(transformNode,
                                              "transformNodeMI.loadTransformTrackingDataFromFilePopup(nuke.thisNode())",
                                              "transformNodeMI.loadTransformTrackingDataFromClipboard(nuke.thisNode())",
                                              )

    for dataName in ["translate","rotate","scale","skew"]:
        createTrackingDataOptionKnob(dataName,transformNode)
        
    transformNode['label'].setValue('MochaImport+')
...
def setTransformDataForTransformNode(transformNode,tfData):
    ...
    if tfData.hasScaleData() and transformNode["importscaledata"].getValue():
        ...
        transformNode["scale"].setValue(1,1)  # make sure the scale has two components, otherwise the next commands cause an error
