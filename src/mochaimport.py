...
def createStabilizedView():
    ...
    startPrecompNode['tile_color'].setValue(int('%02x%02x%02x%02x' % (0.91*255,0.569*255,0*255,255), 16 ))  #[0.91,0.569,0]
    endPrecompNode['tile_color'].setValue(int('%02x%02x%02x%02x' % (0.91*255,0.569*255,0*255,255), 16 ))  #[0.91,0.569,0]
    ...
    stabilizeBackdrop = nuke.nodes.BackdropNode(xpos = startPrecompNode.xpos() + deltaX/2,
                                                ...
                                                tile_color = int('%02x%02x%02x%02x' % (0.91*255,0.569*255,0*255,255), 16 ),  #[0.91,0.569,0]
...
def createCornerPin():
    ...
    cpNode['tile_color'].setValue(int('%02x%02x%02x%02x' % (0.91*255,0.569*255,0*255,255), 16 ))  #[0.91,0.569,0]
...
"""
    supportedNodeClasses = ['Transform', 'Tracker3', 'Tracker4', 'Group', 'GridWarp3', 'SplineWarp3', 'Roto', 'RotoPaint', 'CornerPinMI', 'StartStabilized']
    ...
    if node.Class() == 'Group' and (node.knob('pin1') == None or node.knob('pin2') == None or node.knob('pin3') == None or node.knob('pin4') == None):
        raise MiUnsupportedNodeTypeError("Can only apply tracking data to groups, if they have the knobs 'pin1', 'pin2', 'pin3' and 'pin4'")
    ...
    if node.Class() == 'Transform':
        ...
        cornerPinKnobs.applyCornerPinDataToALlKnobs(cpData,node['pin1'],
                                                           node['pin2'],
                                                           node['pin3'],
                                                           node['pin4'])
