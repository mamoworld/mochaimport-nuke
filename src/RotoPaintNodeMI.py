...
def createRotoOrRotoPaintNodeMI(nodeType):
    ...
    loadTrackingDataTab.addLoadTrackingDataUI(rotoPaintNode,
                                              "RotoPaintNodeMI.loadTransformMatrixFromFilePopup(nuke.thisNode())",
                                              "RotoPaintNodeMI.loadTransformMatrixFromFromClipboard(nuke.thisNode())")
    ...
    rotoPaintNode['label'].setValue('MochaImport+')
...
def createReferenceFrameUI(node):
    ...
    referenceFrameKnob.setTooltip("at this frame, the shapes will preserve their position when the tracking data is loaded.")
    node.addKnob(referenceFrameKnob)
    
    useCurrentFrameKnob = nuke.Boolean_Knob("useCurrentFrameAsReferenceFrame","use current frame")
    useCurrentFrameKnob.setTooltip("at this frame, the shapes will preserve their position when the tracking data is loaded.")
...
def updateLayerChoiceKnobMICallback():
    ...
    knob = nuke.thisKnob()
    if knob.name() == "curves":
        if node.knob('trackingDataLayerMI'): # avoid getting error messages on normal nodes that don't have this knob
...
def createGridWarpNodeMI():
    ...
    loadTrackingDataTab.addLoadTrackingDataUI(gridWarpNode,
                                              "RotoPaintNodeMI.loadTransformMatrixFromFilePopupGridWarp(nuke.thisNode())",
                                              "RotoPaintNodeMI.loadTransformMatrixFromFromClipboardGridWarp(nuke.thisNode())")
    createReferenceFrameUI(gridWarpNode)
    
    gridWarpNode['label'].setValue('MochaImport+')
    return gridWarpNode
