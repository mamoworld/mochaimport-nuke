...
def createMochaPointCloudFromFbxFile(fbxFilePath):
    ...
    geo['label'].setValue('MochaImport+')
    return geo
...
def createMochaCameraRigFromFbxFile(fbxFilePath):
    ...
    pointCloud['help'].setValue("renders the point cloud imported from the mocha camera track. Each tracked layer contributes 5 points (four corners + center of surface rectangle)")
    cam['help'].setValue("camera imported from mocha camera track")
    worldAxis['help'].setValue("use this node to orient the camera track in your scene as desired (e.g. make the ground plane horizontal etc...)")
    worldTransform['help'].setValue("applies your world transformations from node "+worldAxis.name()+" to the point cloud")
...
def mamoBackDrop():
    ...
    if not selNodes:
        return nuke.nodes.BackdropNode(tile_color = int('%02x%02x%02x%02x' % (0.91*255,0.569*255,0*255,255), 16 ),  #[0.91,0.569,0]
