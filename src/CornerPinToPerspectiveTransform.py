...
def cp2TransformMatrix(cpData,referenceFrame):
    ...
    if len(set([len(x1),len(x2),len(x3),len(x4),len(y1),len(y2),len(y3),len(y4)])) > 1:
        raise Exception('all corners must have the same amount of keyframes')
    
    for i in range(0,len(x1)):
        frameNumber = x1[i][0]
        # check times of all 4 entries are equal
        if len(set([x1[i][0],x2[i][0],x3[i][0],x4[i][0],y1[i][0],y2[i][0],y3[i][0],y4[i][0]])) > 1:
            raise Exception('times for keyframes of all four corners must be identical')
        
        matrix = projectiveTransformation.computeProjectionMatrix4FromCornerPin(x1[referenceIndex][1],y1[referenceIndex][1],x1[i][1],y1[i][1],
...
def getFrameIndex(pointValues,frameIndex):
    for i in range(0,len(pointValues)):
        if(pointValues[i][0]==frameIndex):
            return i
    raise Exception("tracking data doesn't contain tracking data for frame "+str(frameIndex)+".\nPlease choose another reference frame.")