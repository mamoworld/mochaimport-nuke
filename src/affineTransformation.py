...
#2D transformation matrices are represented as tuple (a1,a2,a3,a4,a5,a6) which correspond to this transformation matrix:
# a1 a2 a3
# a4 a5 a6 
#  0  0  1
...
def affineTransformFromMovementOfThreePoints(p1Before,p2Before,p3Before, p1After, p2After, p3After):
    ...
    a1 = ((y11-y21)*(x12-x32)-(y11-y31)*(x12-x22)) / ((x11-x21)*(x12-x32)-(x11-x31)*(x12-x22));
    a2 = ((y11-y21)*(x11-x31)-(y11-y31)*(x11-x21)) / ((x12-x22)*(x11-x31)-(x12-x32)*(x11-x21));
    a3 = y11-a1*x11-a2*x12;
    a4 = ((y12-y22)*(x12-x32)-(y12-y32)*(x12-x22)) / ((x11-x21)*(x12-x32)-(x11-x31)*(x12-x22));
    a5 = ((y12-y22)*(x11-x31)-(y12-y32)*(x11-x21)) / ((x12-x22)*(x11-x31)-(x12-x32)*(x11-x21));
